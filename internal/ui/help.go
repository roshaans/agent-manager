...
func helpSections() []helpSection {
	return []helpSection{
		...
		{title: "session under the cursor", rows: [][2]string{
			...
			{"Y", "copy its directory to the clipboard (its worktree, if any)"},
			{"O", "show what it runs: its server in a browser, or a TUI's pane"},
			{"P", "open its pull request, or the repository when it has none"},
			{"i", "its rules: the CLAUDE.md / AGENTS.md this tool reads"},
			{"G", "lazygit on its repository, full screen; quit it to come back"},
		...
		{title: "group under the cursor", rows: [][2]string{
			...
			{"P", "open that repository on the remote it is published to"},
			{"i", "the rules files its projects already carry"},
			{"G", "lazygit on the repository its default path sits in"},
			{"Y", "copy its default path to the clipboard"},
		...
		{title: "rules (i)", rows: [][2]string{
			{"↑↓", "pick a file"},
			{"↵", "read it, or create it when it does not exist yet"},
			{"e", "open it in your editor"},
			{"pgup/pgdn, g/G", "while reading: page, top / bottom"},
			{"esc", "back"},
		}},
		{title: "pull requests (P)", rows: [][2]string{
			{"", "a #number beside a session is the pull request it produced,"},
			{"", "read from what it printed, or from the branch it sits on"},
			{"", "dimmed and marked " + prDraftMark + " while that pull request is a draft"},
			{"", "+1 counts the others when a branch carries more than one"},
			{"↑↓", "with several: pick one"},
