...
func helpSections() []helpSection {
	return []helpSection{
		...
		{title: "session under the cursor", rows: [][2]string{
			...
			{"P", "open its pull request, or offer to open one when it has none"},
		...
		{title: "inside a session (attached or focused)", rows: [][2]string{
			...
			{"shift+←→", "focused: next / previous chat, staying in the pane"},
			{"alt+1…9", "focused: jump straight to that chat, where Option sends Meta"},
